...
class _DetailPesertaDonorPageState extends State<DetailPesertaDonorPage> {
  ...
  final TextEditingController _kantongController = TextEditingController(text: '1');
  ...
  Widget _buildNameSelector(String name) {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(
            child: Text(name, style: const TextStyle(fontSize: 11.5, color: AppColors.textGrey)),
          ),
          const Icon(Icons.keyboard_arrow_down_rounded, size: 18, color: AppColors.textDark),
        ],
      ),
    );
  }

  Widget _buildProfileCard({required String nama, required String kartu, required String goldar}) {
    return Container(
      width: double.infinity,
  ...
  Widget _buildInfoStrip({required String kartu, required String goldar}) {
    return Container(
      ...
      child: Row(
        children: [
          Expanded(child: Text(kartu, style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.w600, color: Colors.black87))),
          Container(width: 1, height: 18, color: Colors.grey.shade400),
          const SizedBox(width: 12),
          const Text('Golongan Darah : ', style: TextStyle(fontSize: 11, color: Colors.black45)),
          Text(goldar, style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.bold, color: Colors.black87)),
        ],
      ),
    );
  }
      ...
      children: [
        ...
        SizedBox(
          ...
          child: TextField(
            ...
            keyboardType: TextInputType.text, // Memastikan keyboard angka yang muncul
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: () {
          ...
          returnData['jumlahKantong'] = inputKantong.isEmpty ? '1' : inputKantong;
        ...
        child: const Text('Tambah', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600)),
      ),
    );
  }
}